...
- [ ] **A1-1** `cd ~/project/Agent/Agent_skills && git init -b main`
...
- [ ] **A2-2** 原子搬迁：`mv ~/.config/opencode/skills-portal ~/project/Agent/Agent_skills/portal`
  - 验证：`ls Agent_skills/portal/bin/start` 存在；`ls ~/.config/opencode/skills-portal` 报 "No such file"
- [ ] **A2-3** 建反向软链接保留兼容：`ln -s ~/project/Agent/Agent_skills/portal ~/.config/opencode/skills-portal`
...
- [ ] **B4-1** 写 `~/project/Agent/Agent_skills/AGENTS.md`，至少包含：
  - 项目一句话定位
