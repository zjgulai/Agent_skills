...
> **当前状态**：`/Users/lute/project/Agent/Agent_skills/` 空目录。
...
| 路径 | 用途 | 状态 |
|---|---|---|
| [`~/.config/opencode/skills/`](file:///Users/lute/.config/opencode/skills) | 6 个用户级 skill + INDEX.md + skills-graph.mmd + render-mermaid.py | **生产数据，永不删** |
| [`~/.config/opencode/skills-portal/`](file:///Users/lute/.config/opencode/skills-portal) | FastAPI（5174）+ Vite/Vue（5173），含 `.venv` + `node_modules` + `dist/` | **要搬迁，搬迁后用软链接保留旧路径以防其他工具引用** |
| `/Applications/Skills Portal.app` | AppleScript launcher，调用 `~/.config/opencode/skills-portal/bin/start` | **start 路径要保持有效**（靠软链接） |
| [`~/.config/opencode/AGENTS.md`](file:///Users/lute/.config/opencode/AGENTS.md) | 全局规则，知识库依赖 ~/lute_knowledge | 项目级 AGENTS.md 优先级更高，但本计划不冲突 |
...
```bash
# 1. 实际搬动（mv 是原子的）
mv ~/.config/opencode/skills-portal /Users/lute/project/Agent/Agent_skills/portal

# 2. 软链接保留旧路径，让 /Applications/Skills Portal.app 和其他引用照常工作
ln -s /Users/lute/project/Agent/Agent_skills/portal ~/.config/opencode/skills-portal
...
[02-execution-todo.md](file:///Users/lute/project/Agent/Agent_skills/.sisyphus/plans/02-execution-todo.md) 给出按 10 个原子任务展开的 28 条勾选项，每条都有：
- 触发条件 / 输入
- 执行动作（命令 / 文件路径 / 写什么）
- 验证手段（怎么知道这一步真的做完了）
- 回退方案（出错怎么撤）
