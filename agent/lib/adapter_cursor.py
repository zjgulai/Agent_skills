...
CLIENT_NAME = "cursor"
SKILLS_DIR = Path.home() / ".cursor" / "skills-cursor"
...
def install(m: Manifest) -> dict:
    ...
    return {"client": CLIENT_NAME, "name": m.name, **make_symlink(target, _link_path(m.name))}
...
def uninstall(m, name=None) -> dict:
    ...
    if not link.is_symlink():
        return {"client": CLIENT_NAME, "name": n, "removed": False, "reason": "not-a-symlink-or-not-found"}
    if m:
